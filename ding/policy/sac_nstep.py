...
@POLICY_REGISTRY.register('sac_nstep')
class SACNstepPolicy(SACPolicy):
    r"""
       Overview:
    ...
    r"""
    Overview:
        Policy class of SAC with model-based nstep policy gradient.
    """
    ...
    def _init_learn(self) -> None:
        ...
        """
        super()._init_learn()
        self._n_step = self._cfg.learn.n_step
        self._n_step_norm = self._cfg.learn.n_step_norm
        self._beta = torch.tensor(
            [self._cfg.learn.get('beta', 0.0)], requires_grad=False, device=self._device, dtype=torch.float32
        )
    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        if self._value_network:
            ...
            with torch.no_grad():
                ...
                next_log_prob = dist.log_prob(pred) + 2 * torch.log(torch.cosh(pred)).sum(-1)
                ...
                if self._twin_critic:
                    ...
                    target_q_value = torch.min(target_q_value[0],
                                               target_q_value[1]) - self._alpha * next_log_prob
                else:
        ...
        # when self._n_step is 0, it is naive sac
        policy_loss = 0
        ...
        eval_data = {'obs': obs, 'action': action}
        new_q_value = self._learn_model.forward(eval_data, mode='compute_critic')['q_value']
        # _, _  = self._env_model.batch_predict(obs, action)
        if self._twin_critic:
            new_q_value = torch.min(new_q_value[0], new_q_value[1])
        policy_loss += (self._gamma ** self._n_step) * (self._alpha * log_prob - new_q_value).mean()

        if self._n_step_norm:
            policy_loss = policy_loss / (self._n_step + 1)
